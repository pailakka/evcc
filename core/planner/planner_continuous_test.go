...
func TestContinuous_CheapestContiguousSlots(t *testing.T) {
	...
	plan := p.Plan(2*time.Hour, now.Add(6*time.Hour), testStrategy(true, 0))
...
func TestContinuous_WindowWithPastRates(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, targetTime, testStrategy(true, 0))
...
func TestContinuous_WindowAllRatesInPast(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, targetTime, testStrategy(true, 0)) // continuous mode
...
func TestContinuous_TargetAfterKnownPrices(t *testing.T) {
	...
	plan := p.Plan(40*time.Minute, clock.Now().Add(2*time.Hour), testStrategy(true, 0)) // charge efficiency does not allow to test with 1h
	assert.False(t, !SlotAt(clock.Now(), plan).IsZero(), "should not start if car can be charged completely after known prices ")

	plan = p.Plan(2*time.Hour, clock.Now().Add(2*time.Hour), testStrategy(true, 0))
...
func TestContinuous_Precondition(t *testing.T) {
	...
	plan := p.Plan(tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), testStrategy(true, tariff.SlotDuration))
	...
	plan = p.Plan(2*tariff.SlotDuration, clock.Now().Add(4*tariff.SlotDuration), testStrategy(true, tariff.SlotDuration))
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now(),
	...
	plan = p.Plan(time.Duration(1.5*float64(tariff.SlotDuration)), clock.Now().Add(4*tariff.SlotDuration), testStrategy(true, tariff.SlotDuration))
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now(),
	...
	plan = p.Plan(tariff.SlotDuration, clock.Now().Add(time.Hour), testStrategy(true, 24*time.Hour))
	assert.Equal(t, api.Rates{
		{
			Start: clock.Now().Add(3 * tariff.SlotDuration),
			End:   clock.Now().Add(4 * tariff.SlotDuration),
...
func TestContinuous_Precondition_NonSlotBoundary(t *testing.T) {
	...
	plan := p.Plan(requiredDuration, targetTime, testStrategy(true, precondition))
...
func TestPrecondition_Everything(t *testing.T) {
	...
	planContinuous := p.Plan(requiredDuration, targetTime, testStrategy(true, precondition))
...
func TestContinuous_ContinuousPlanNoTariff(t *testing.T) {
	...
	plan := p.Plan(time.Hour, clock.Now(), testStrategy(true, 0))
...
func TestContinuous_ContinuousPlan(t *testing.T) {
	...
	plan := p.Plan(150*time.Minute, clock.Now(), testStrategy(true, 0))
...
func TestContinuous_ContinuousPlanOutsideRates(t *testing.T) {
	...
	plan := p.Plan(30*time.Minute, clock.Now(), testStrategy(true, 0))
...
func TestContinuous_ExcessTimeFinishesAtTarget(t *testing.T) {
	...
	plan := planner.Plan(requiredDuration, targetTime, testStrategy(true, 0)) // continuous, no precondition
	...
	plan = planner.Plan(requiredDurationShort, targetTime, testStrategy(true, 0)) // continuous, no precondition
	...
	plan = planner.Plan(requiredDurationMedium, targetTime, testStrategy(true, 0)) // continuous, no precondition
